...
// end citation
 ...
 void GLFunctions::cylinderFrom(Vec4 _from, Vec4 _dir,float _radius,const float _height, int _slices, int _stacks )
 {
   /* Step in z and radius as stacks are drawn. */
   _dir.normalize();
   double z0,z1;
   ...
   Vec4 up(0,1,0);
   Vec4 xaxis = up.cross(_dir);
   xaxis.normalize();

   Vec4 yaxis = _dir.cross( xaxis);
   yaxis.normalize();
   std::cout<<"yaxis "<<yaxis.m_x<<" "<<yaxis.m_y<<" "<<yaxis.m_z<<"\n";
   std::cout<<"xaxis "<<xaxis.m_x<<" "<<xaxis.m_y<<" "<<xaxis.m_z<<"\n";
   // not stuff into mat4 for rotation
   Mat4 r;
  r.m_00  = xaxis.m_x;
  r.m_01  = yaxis.m_x;
  r.m_02  = _dir.m_x;
  // may as well put offset in
  //r.m_03  = _from.m_x;
  ...
  //r.m_23  = _from.m_z;
   ...
   for(int i=1; i<=_stacks+1; ++i )
   {
     ...
     for(int j=0; j<=_slices-1; ++j)
     {
       ...
       glTexCoord2f(s,t);
       glNormal3f(nx,ny,nz);
       point.m_x=x; point.m_y=y; point.m_z=z;
       point= point *r;
       glVertex4fv(&point.m_openGL[0]);
       // vert 2
       t=v+dv;
       z=-z1/2.0;
       glTexCoord2f(s,t);
       ...
       point= point *r;
       glVertex4fv(&point.m_openGL[0]);
       //std::cout<<"xaxis "<<point.m_x<<" "<<point.m_y<<" "<<point.m_z<<"\n";
